...
| `message_photo_url` | String (URL) | Optional - URL изображения для рассылки |
...
- **Из Directus:** Upload файл в Directus и используй `https://your-directus.com/assets/{file-id}`
- **Telegram File ID:** `AgACAgIAAxkBAAI...` (если файл уже загружен в Telegram)

> 💡 **Важно:** Бот автоматически определит, есть ли изображение. Если `message_photo_url` заполнено - отправит фото с подписью, иначе - просто текст.
...
#### Управление изображениями в сообщениях

Для добавления изображений к системным сообщениям (например, к приветствию или справке), используйте специальные ключи:
...
```json
{
  ...
  "value": "https://your-directus.com/assets/welcome-image.jpg",
  "description": "Изображение для приветствия (опционально)"
}
```
...
{
  "key": "msg.help_photo",
  "value": "https://example.com/help-guide.png",
  "description": "Изображение для команды /help (опционально)"
}
```

**Поддерживаемые форматы URL:**
- Прямые ссылки на изображения: `https://example.com/image.jpg`
- Directus Assets: `https://your-directus.com/assets/{file-id}`
- Telegram File ID: `AgACAgIAAxkBAAI...` (повторное использование уже загруженных файлов)

**Как использовать в боте:**
1. Создайте ключ `msg.{message_name}_photo` в app_settings
2. Бот автоматически проверит наличие этого ключа при отправке сообщения
3. Если фото найдено - отправит `sendPhoto()` с текстом в caption
4. Если фото нет - отправит обычное текстовое сообщение
...
2. **Webhook / Request URL** - Вызвать бота
   ...
   Body:
   {
     ...
     "message_photo_url": "{{read_data.message_photo_url}}",
     "target_audience": "{{read_data.target_audience}}"
   }
   ```
   
   > 💡 **Важно:** Если вы используете файл из Directus, `message_photo_url` будет автоматически содержать полный URL вида `https://your-directus.com/assets/{file-id}`
